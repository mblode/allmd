...
```bash
npm install              # setup (requires Node >= 24)
...
- **Docs assets track blode.md, not this repo**: `apps/web/lib/docs-proxy.ts` proxies `/allmd/docs/*` to `allmd.blode.md` and rewrites the upstream's root-absolute asset URLs so they stay inside the zone prefix. The segment it matches is the platform's Next.js `assetPrefix`, so it tracks blode.md. It moved from `/_next` to `/_docs` silently and every docs page lost its CSS and JS until this caught up. Chunks are also served only from the apex `blode.md`; the tenant host `allmd.blode.md` emits those URLs but 404s on them. If the docs render unstyled, diff the upstream HTML's asset paths against `ASSET_SEGMENT` before looking anywhere else.
- **Docs `<head>` metadata is rewritten too**: the upstream points `llms.txt`, `llms-full.txt`, the manifest and the icons at root-absolute paths. Under the proxy those resolve to blode.co's own copies and answer 200, so nothing 404s and no crawler flags it, but an allmd docs page ends up advertising the personal site's files. `ROOT_URL_REWRITES` in `apps/web/lib/docs-proxy.ts` repoints them. If the platform adds another root-absolute `<head>` link, it needs an entry there, and the target has to be a URL that already exists: `/allmd/docs/manifest.json` is a 404, and `/allmd/llms.txt` is a 200 but it is the marketing index rather than the docs index.
