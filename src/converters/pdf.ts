...
export async function convertPdf(
  ...
  verbose(`PDF size: ${Math.round(buffer.byteLength / 1024)} KB`, options.verbose);

  const parsed = await pdfParse(buffer);
  verbose(`Parsed ${parsed.numpages} pages, ${parsed.text.length.toLocaleString()} chars extracted`, options.verbose);
  ...
  if (hasText) {
    ...
    verbose("PDF appears to be scanned/image-based, skipping AI formatting", options.verbose);
    markdown =
      `# ${filename}\n\n` +
